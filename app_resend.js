...
async function getAccountsNonces (){
	for(let i = 0 ; i < accounts.length; i++){
		...
		accounts[i].startnonce = accounts[i].nonce;
		nonceTracker[accounts[i].addr].add(accounts[i].nonce);
	}
...
getAccountsNonces().then(()=>{
	...
	var loop = ()=>{
		...
		let startNonces = new Array(accounts.length);
		let currentRound = round;
		accounts.forEach((acc,index)=>{
			startNonces[index] = acc.nonce;
		})
		//let dupTxChecker = {};
		// accounts.forEach((acc)=>{
		...
		return Promise.all(txCollection).then((resps)=>{
			...
			console.log("[Connection No Error "+round+"-"+currentRound+"]");
			console.log(startNonces);
			let onError = false;
			...
			console.log("[Connection Error "+round+"-"+currentRound+"]"+ error);
			console.log(startNonces);
			console.log(accounts);
			loops.forEach((lp)=>{
				clearInterval(lp.interval);
			...
			round = currentRound;
			accounts.forEach((acc,index)=>{
					accounts[index].nonce = accounts[index].nonce> startNonces[index]?startNonces[index] :accounts[index].nonce;
				});
...
var closeProcessHandler = ()=>{
	...
	accounts.forEach((acc,index)=>{
			...
			console.log(str);
			totalTxCount += nonceTracker[acc.addr].size;
		});
	//provider.closeConnections();

	console.log("\n[Total Transaction Counts]\t"+ totalTxCount);
	console.log("[Total runtimes]"+runtimes);
	if(loops != undefined){
		loops.forEach((lp)=>{
					clearInterval(lp.interval);
				})
